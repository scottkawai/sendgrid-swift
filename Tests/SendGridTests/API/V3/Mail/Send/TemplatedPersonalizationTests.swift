...
class TemplatedPersonalizationTests: XCTestCase, EncodingTester {
    typealias EncodableObject = TemplatedPersonalization<DynamicData>
    
    struct DynamicData: Encodable {
        let foo: String
        let bar: [Int]
    }
    ...
    func generateExample(_ isSimple: Bool = true) -> TemplatedPersonalization<DynamicData> {
        ...
        if isSimple {
            return TemplatedPersonalization(dynamicTemplateData: self.dummyTemplateData,
                                            to: recipients)
        } else {
            let ccs = self.generateRecipients(2, prefix: "copy")
            let bccs = self.generateRecipients(2, prefix: "blind_copy")
            return TemplatedPersonalization(
                dynamicTemplateData: self.dummyTemplateData,
                to: recipients,
                cc: ccs,
                bcc: bccs,
    ...
    func testEncoding() {
        let basic = TemplatedPersonalization(dynamicTemplateData: self.dummyTemplateData, recipients: "foo@example.none", "bar@example.none")
        XCTAssertEncodedObject(basic, equals: ["to": [["email": "foo@example.none"], ["email": "bar@example.none"]], "dynamic_template_data": ["foo": "foobar", "bar": [1, 2, 3]]])
        
        let complete = TemplatedPersonalization(
            dynamicTemplateData: self.dummyTemplateData,
    ...
    func testInitialization() {
        ...
        let good = TemplatedPersonalization(dynamicTemplateData: self.dummyTemplateData, recipients: "test1@example.none", "test2@example.none")
    ...
    func testValidation() {
        do {
            let missing = TemplatedPersonalization(dynamicTemplateData: self.dummyTemplateData, to: [])
        ...
        do {
            let badTo = TemplatedPersonalization(dynamicTemplateData: self.dummyTemplateData, recipients: "test")
            try badTo.validate()
        ...
        do {
            let bad = TemplatedPersonalization(dynamicTemplateData: self.dummyTemplateData, recipients: "test1@example.none")
            bad.headers = [
                "X-CUSTOM": "FOO",
